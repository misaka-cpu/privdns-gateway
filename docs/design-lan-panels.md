...
>
> **端到端已在真机上跑通**(2026-08-22, iPhone 零 App 打开七个面板)。那一轮暴露了三个
> 沙盒查不出来的 bug —— 见第 9 节, 那份记录比"验过了"三个字有用得多。
...
WLOC 是 940 行 + 7 支(该功能此后已退役)—— 所以这个功能**比救援平面小**,与当时的 WLOC 相当。
...
### 端到端实测(2026-08-22, 生产网关 + 真 iPhone)

链路: iPhone(蜂窝, **零 App**) → jp2 → tailnet → 家里七台设备。七个面板全部打开,
证书是 Let's Encrypt 真证书, acme.sh 的 DNS-01 签发也是真跑的。

**这一轮暴露了三个沙盒查不出来的 bug**, 记在这里比"验过了"三个字有用:

| 症状(网关侧) | 真相(手机侧) | 根因 |
|---|---|---|
| `pdg lan cert` 打印"✅ 证书已就位" | 六个面板证书错误 | acme.sh 一次签发多域名产出的是**一张** SAN 证书, 却按域名逐个去装 —— 除第一个外全失败 |
| curl 到 UPS 回 302, 一切正常 | UPS 回 404 | Referer 写成了**无条件设置**。浏览器首次导航本不带 Referer, 硬塞一个进去等于伪造来源 |
| curl 到 sub-store 回 302, 一切正常 | "重定向次数过多" | 入口参数的跳转判据只写了"路径是根", 而跳完路径**仍然是根** —— 自己跳自己 |

三个的共同点: **从网关自己 curl 一下全都看着正常。**后两个还有更值得记的一点 ——
正确写法本来就在方案 A 那套跑了很久的配置里, 移植时搬过来的是"要改哪几个头",
没搬"怎么改", 而后者才是在真设备上踩出来的部分。A 的注释里白纸黑字写着"否则会自己跳
自己", 读过了还是漏了判据。

### 仍然没有验过的

- **ACL 越界探测**打在真 tailnet 上的表现。判据的三种读法有单元测试覆盖(连上 / 被拒 /
  不可达), 但作者的 tailnet 上目前是默认全通(他判断 jp2 被拿下的概率低, 接受了风险①),
  所以这一项在真 ACL 上没跑过。
- **续期**。acme.sh 装的 cron 是否真的把证书续上, 没有跨越 60 天验证过。
  doctor 的证书项就是为这条不确定性存在的。
- **别人的设备**。七台是作者家里那些(华为 UPS、Zyxel 交换机、TrueNAS、IPMI、ESXi、
  路由器、sub-store)。别的型号会有别的毛病 —— 生成器现在带的五条改写能力就是这么攒出来的。
