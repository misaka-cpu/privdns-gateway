...
| 443 | tcp+udp | 仅内网卡段 | **mihomo 数据入口(nft REDIRECT → redir 监听,嗅 SNI / QUIC)** |
| 80 | tcp | 仅内网卡段 | mihomo HTTP 入口(嗅 Host) |
...
⚠️ **证书签发/续期需要从公网访问 80 端口**(Let's Encrypt HTTP-01 校验):签发时 pre-hook 会把 80 临时对全网开放(并停内核),完后还原。
