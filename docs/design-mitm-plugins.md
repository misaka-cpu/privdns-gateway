...
> 状态:**阶段 1-5 已实现并在 .200 真机集成验证;仅差真 iPhone 最终验收(阶段 6)**。仿 5GPN-X 的「声明接管域名 → 网关执行动作 → 出口仍由内核决定」。首个插件:Apple WLOC(把 Apple 的 Wi-Fi 定位响应改写成指定坐标)。

## 使用(iOS 安装)

1. 装机时选 iOS(`PDG_PLATFORM=ios`)→ 自动装 `pdg-mitm` 服务。
2. bot「🛠 运维 → 🍏 位置改写(WLOC)」→「📍 设坐标」发「纬度,经度」→「开启」。
3. bot「📱 客户端 → iOS 描述文件」装到 iPhone(此时描述文件已内含网关 CA)。
4. **iPhone 手动信任 CA**:设置 → 通用 → VPN与设备管理(装描述文件)→ 关于 → 证书信任设置 → 对「PrivDNS Gateway MITM CA」开启完全信任。

> ⚠️ **信任代价**:设备信任这张 CA = 它能解密该设备**所有 HTTPS**。系统只对声明的接管域名(gs-loc.apple.com)实际 MITM,其余不解密,但能力是广的。仅用于自己的设备。

## 阶段 6 真机验收清单(需你的 iPhone)

- [ ] iPhone 装描述文件 + 信任 CA 后,定位是否真的落到设定坐标(地图 App 看)。
- [ ] 若不变:抓 `gs-loc.apple.com` 的真实请求/响应包,核对 `mitm_wloc._split_header` 的头部字节是否与实现一致,按真机校准。
- [ ] 关闭 WLOC 后定位恢复正常。
- [ ] 验收通过后:CHANGELOG + 合并 main + 发 v1.5.0。
