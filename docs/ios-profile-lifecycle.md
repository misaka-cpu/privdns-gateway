...
任何办法知道,而用户知道。Bot 给三个按钮(装过 / 从未装过 / 取消),CLI 问一行
(`PDG_IOS_LEGACY` 供非交互场景显式给出)。猜错的代价是双向的:猜"装过"会让用户白跑一趟去
删一个根本不存在的描述文件,猜"没装过"会让手机上悄悄多出一个永远不会被更新的配置——两头
都不报错。

**Bot 的两条入口都要问**:点「生成 / 更新描述文件」是一条,先发 SSID 名单也是一条。后者
以前从"记录里没有 current"直接推断成"以前装过"并立刻生成;现在改成把名单先收下暂存,得到
回答之后才生成。**没得到回答就一个字节都不写**——不建记录、不落产物、不占用身份;取消、
返回、回主菜单、切页、`/cancel`、平台不是 iOS,都会把暂存清掉。

## 2. 稳定身份

- 首次启用时生成一次 `instance_id`(`uuid4`),持久化在元数据里,**永不再变**;
- 所有 payload 的 **UUID** 由 `uuid5(NS, instance_id + ":" + 角色)` 派生:
  角色固定为 `root` / `dns` / `ca`;
- **identifier 的规则不一样,别混为一谈**:
  - 顶层与 DNS payload 是 `com.privdns.gateway.<uuid_root>` / `com.privdns.gateway.dot.<uuid_dns>`
    —— 把派生 UUID 拼在后面,于是每台网关各不相同;
  - 根证书 payload 是**固定的** `com.privdns.mitm.ca`,只有它的 UUID 随 `instance_id` 变。

  这不是遗漏。Apple 的匹配语义里,**profile 的身份是顶层的 identifier + UUID**;payload 的
  identifier 只在这份 profile 内部要求唯一,不跨 profile 生效。两台网关的顶层 identifier 已
  经不同 ⇒ 它们是两份不同的描述文件,各自带各自的根证书 payload,谁也替换不掉谁;同一台网
  关重新生成时,顶层身份不变 ⇒ iOS 视为"同一份的新版本",根证书那一格照样跟着更新。
  所以把它改成派生形式**换不来任何隔离**,代价却是实打实的:`ID_CA` 参与渲染字节,改了它
  之后所有已存在版本的 `sha256` 都对不上,而语义输入没变 ⇒ `digest` 不变 ⇒ 判定仍说"无需
  更新",于是 `repair_current` 会永久拒绝复原(重新渲染的结果与记录不符),用户手上那份也
  不会被任何提示叫去更新。结论:**保持固定,改文档而不是改常量**。
