...
# 证据的留存期。**必须严格覆盖会话窗口, 外加一整段结算余量** ——
#     会话窗口 [t0, t1], t1 = t0 + 300(会话模块的 TTL_SECS)
#     600 = 300(窗口) + 300(结算余量)
# 为什么不能和会话窗口一样长: 那样窗口最早那一刻写下的证据恰好在窗口关闭那一刻到期,
# 而"没观测到"这个结论的前提是"假如它来过现在还看得见"(now <= t0 + 留存期) —— 那正好
# 等于"窗口还开着"。于是窗口开着只能说"还在等", 窗口一关就只能说"不知道",
# NOT_OBSERVED 在任何时刻都不可达, 而那恰恰是这个功能要给的主要答案。
# 300 秒余量的含义: 用户测完之后再花一整个测试的时间来看结果, 都还来得及。
# 隐私代价是证据最多多留 5 分钟, 内容不变(仍只有随机 label 的 SHA256、时间、qtype、
# transport), 且 /run 上的东西停服即消失。
# 不做成环境变量: 它是判定语义的一部分, 让部署方能改等于让"什么算证据"因机器而异。
# 也不去会话模块里读那个 300 —— 这个进程直面网络输入, 依赖越少越好(隐私门 15 就
# 盯着这条: 本文件里出现会话模块的导入语句即判失败)。两个常量的关系由
# tests/test-dot-ttl.py 跨模块守住, 而不是靠运行时耦合。
