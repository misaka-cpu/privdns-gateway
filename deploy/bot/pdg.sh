...
# ── 服务前像: 保存与恢复 ─────────────────────────────────────────────────────
# 为什么需要它: 快照只收文件。而 `/etc/systemd/system/<u>.service` 恢复回来, **不等于**
# 那个服务回到了原来的运行态与自启态 —— enable 的符号链接在
# multi-user.target.wants/ 下, 全仓从不快照它。于是"文件都回来了、服务却停着且不自启"
# 这种现场会被回滚自报成"已回滚并重启服务"。这一组就是补这一格。
#
# 三条纪律:
#   · active 与 enabled 是**两件独立的事**, 分别记、分别恢复;
#   · 查不到 ≠ 没有。查询失败写 QUERY-FAILED, 绝不写成空串再让两个空串相等;
#   · 记录**不是**通用凭据, 它只服务于"这一次操作的回滚", 所以要和本次操作绑死。

# 要保前像的服务集合。**固定一份**, 不按平台裁剪 —— 少记一个就等于那一格无法恢复。
_pdg_svcstate_units(){
  printf '%s\n' pdg-mitm pdg-bot pdg-probe81 mosdns mihomo pdg-dotwitness \
...
# 把**操作前**的服务状态写进本次快照目录。
# 头部记的是"这条记录属于谁、属于哪一次": boot_id + 持锁进程的 pid/starttime + 快照目录。
# 尾部记条数与正文摘要 —— 截断、少行、重复都能当场看出来。
_pdg_save_svcstate(){   # $1=本次快照目录
  local sd="$1" f body u ufs asv sub inv n=0 st tb
  [[ -d "$sd" ]] || { c_y "❌ 服务前像: 快照目录不存在: $sd"; return 1; }
  tb="$sd/snap.tar.gz"
  # 前像必须写在**回滚真正会用到的那份快照**旁边, 且把那份快照的身份记进来。
  # 否则"记录是这一次的"与"快照是这一次的"是两件没有关联的事。
  [[ -f "$tb" ]] || { c_y "❌ 服务前像: 快照目录里没有 snap.tar.gz: $sd"; return 1; }
  f="$sd/svcstate.tsv"
  body="$(mktemp)" || return 1
  {
    ...
    printf 'holder_pid\t%s\n'   "$$"
    # /proc/<pid>/stat 第 22 个字段是进程启动时刻(自开机起的 tick)。pid 会复用, pid+starttime 不会。
    printf 'holder_start\t%s\n' "$(awk '{print $22}' "/proc/$$/stat" 2>/dev/null)"
    printf 'snap_dir\t%s\n'     "$sd"
    # 快照身份: 设备:inode + 字节数 + mtime。回滚会用这一份; 记录就钉在这一份上。
    printf 'snap_id\t%s\n'      "$(stat -c '%d:%i:%s:%Y' "$tb" 2>/dev/null)"
    printf 'created_at\t%s\n'   "$(date -u +%FT%TZ)"
    while read -r u; do
      [[ -n "$u" ]] || continue
      ufs="$(_pdg_svc_q is-enabled "$u")"
      asv="$(_pdg_svc_q is-active  "$u")"
      sub="$(systemctl show -p SubState     --value "$u" 2>/dev/null)"
      inv="$(systemctl show -p InvocationID --value "$u" 2>/dev/null)"
      printf 'unit\t%s\t%s\t%s\t%s\t%s\n' "$u" "$ufs" "$asv" "${sub:-QUERY-FAILED}" "${inv:-}"
      n=$((n+1))
    done < <(_pdg_svcstate_units)
  } > "$body" || { rm -f "$body"; return 1; }
  n="$(grep -c $'^unit\t' "$body")"
  st="$(sha256sum "$body" | awk '{print $1}')"
  { cat "$body"; printf 'end\t%s\t%s\n' "$n" "$st"; } > "$f" || { rm -f "$body"; return 1; }
  rm -f "$body"
  chmod 600 "$f" || return 1
  c_g "  已保存服务前像($n 个 unit): $f"
}

# 读并**校验**一份前像。$1=记录路径; 校验不过就说清是哪一条不过。
# 只做结构与完整性校验 —— "这条记录是不是本次操作的"由调用方另判(见退役版的调用方门)。
_pdg_svcstate_valid(){   # $1=文件 → 0=可用
  local f="$1" tail cnt want got body
  [[ -f "$f" ]] || { _PDG_SVCSTATE_WHY="记录不存在"; return 1; }
  head -1 "$f" | grep -qx $'#pdg-svcstate\t1' || { _PDG_SVCSTATE_WHY="不是认识的前像格式/版本"; return 1; }
  tail="$(tail -1 "$f")"
  [[ "$tail" == end$'\t'* ]] || { _PDG_SVCSTATE_WHY="记录没有结尾行(可能被截断)"; return 1; }
  cnt="$(cut -f2 <<<"$tail")"; want="$(cut -f3 <<<"$tail")"
  body="$(mktemp)" || return 1
  head -n -1 "$f" > "$body"
  ...
  n_need="$(_pdg_svcstate_units | grep -c .)"
  [[ "$n_have" == "$cnt" ]]   || { _PDG_SVCSTATE_WHY="服务行数与结尾行不符($n_have vs $cnt)"; return 1; }
  [[ "$n_have" -gt 0 ]]       || { _PDG_SVCSTATE_WHY="一条服务记录都没有"; return 1; }
  [[ "$n_have" == "$n_uniq" ]]|| { _PDG_SVCSTATE_WHY="有重复的 unit 记录"; return 1; }
  local miss="" u
  while read -r u; do
    [[ -n "$u" ]] || continue
    grep -q "^unit"$'\t'"$u"$'\t' "$f" || miss="$miss $u"
  done < <(_pdg_svcstate_units)
  [[ -z "$miss" ]] || { _PDG_SVCSTATE_WHY="缺少必需服务的前像:$miss"; return 1; }
  [[ "$n_uniq" == "$n_need" ]] || { _PDG_SVCSTATE_WHY="服务集合与本版清单不一致($n_uniq vs $n_need)"; return 1; }
  return 0
}
...
cmd_rollback(){
  ...
  # ── 路径表示统一一次 ────────────────────────────────────────────────────────
  # 序号那条路走的是 `ls -1dt "$SNAP_DIR"/*/`, 它给出的每一项都**带末尾斜杠**; 而服务前像里
  # 记的 snap_dir 是 cmd_snapshot 当时的 "$SNAP_DIR/$ts"(不带)。两边直接比字符串, 于是
  # `pdg rollback 0` 回滚自己刚拍的那一份也会被判成"这份前像记的是别的快照" —— 同一个目录,
  # 只因为一个斜杠。--dir 由调用方给, 带不带斜杠都可能, 同样要统一。
  # 这里**只统一表示**: 去掉末尾斜杠, 不做符号链接解析(符号链接策略保持原样),
  # 也不放宽后面的任何校验 —— 目录绑定、tarball 身份、损坏记录该拒的照拒。
...
_update_mosdns_preflight(){
  ...
  (
    ...
    # ── 到这里就是"不合法"。下面只为把原因说清楚 —— 但**摘要没过之前一次都不执行它** ──
    # 这一段过去为了把原因分成"版本漂移"和"内容不符", 在判据失败之后又跑了两次
    # `"$bin" version`: 判据挡在门外的文件, 诊断又把它请了进来, 而且是在 root 的更新预检里,
    # 面对的恰恰是最可能被动过手脚的那台机器。顺序改成先把内容证明完, 才允许执行。
    [[ -e "$bin" ]]                     || exit 1   # 不存在
    [[ -f "$bin" ]]                     || exit 7   # 不是普通文件(目录/设备/指向异常的链接)
    [[ -x "$bin" ]]                     || exit 2   # 在那儿但执行不了
    [[ -n "${PDG_SHA256[mosdns-bin-$march]:-}" ]] || exit 11   # 本架构没有钉值 → 无从对照
    local got_sha
  ...
  [[ "$rc" == 0 ]] && return 0
  march=$(dpkg --print-architecture 2>/dev/null); [[ "$march" == arm64 ]] || march=amd64
  # ── 摘要不符**统一判为不可信内容**, 不再区分"版本漂移" ─────────────────────
  # 旧契约(v1.11.9)是: 自报版本与钉值不符 → 当成"这次更新会顺手修好的版本漂移", 放行。
  # 那条放行有两个问题, 本轮一并撤掉:
  #   ① 它要先**执行**那个文件才能读到自报版本 —— 而"要不要信这个文件"正是当时还没回答的问题;
  #   ② 自报版本是**文件自己说的**。一个被替换过的二进制想说什么版本就说什么版本, 于是
  #      "版本漂移"这条放行通道对真正需要拦住的那类文件恰好是敞开的。
  # 代价是有意接受的: 手工换过内核、摘要不属于当前正式版的机器, 不再被例行 update 自动抹平,
  # 用户必须先恢复可信内核。反过来的默认(让例行更新顺手覆盖一个来路不明的二进制)会把这台
  # 机器上最该报警的一件事变成一行没人看的日志。
  #
  # 正常升级不受影响: 预检跑在 reset **之前**, 读的是旧 checkout 的 versions.sh。一台同步的
  # 旧版机器上, 旧钉值与盘上那个旧二进制本来就一致 → rc=0 直接过, 换新钉值发生在 reset 之后。
  c_y "❌ 更新前自检: mosdns 二进制不合法, 拒绝开始更新。"
  case "$rc" in
    1)  echo "  $bin **不存在**(缺失 —— 核心解析器不在, 这台机器现在是坏的)";;
    2)  echo "  $bin 在那儿但**执行不了**(权限 / 不是可执行文件)";;
    3)  echo "  $bin version **命令非零**(内容对得上钉值, 但它起不来)";;
    4)  echo "  $bin version 的输出里**读不出版本号**(内容对得上钉值, 输出却不是预期形态)";;
    5)  echo "  **自报版本与钉值不符**: 内容摘要等于钉值, 自报版本却不是 $MOSDNS_VER —— 钉值与资产自相矛盾, 不在存疑时动手";;
    6)  echo "  **SHA256 摘要不符**: 文件内容不是本机仓库钉死的那一份 —— 按不可信内容处理";
        echo "  不再按自报版本区分「版本漂移」: 那要先执行这个文件才读得到, 而它说什么版本都由它自己决定。";
...
cmd_update(){
  ...
  # 把本次前像的**句柄**交给迁移子进程。句柄本身不是证据 —— 新版会去验它指向的那份记录
  # 是否属于本次操作(见退役版的调用方门)。旧版迁移看不懂这个变量, 也不受影响。
...
# 服务进程把自己的身份写在**会话目录里**, 而不是只写进运行期记录。原因是记录要等 HTTP 起来
# 之后才落盘, 而目录在那之前很久就存在了 —— 强杀正好落在那段真空里的话, 盘上有一个带凭据的
...
# 父目录可信判据。**一律不跟随符号链接**, 而且只描述"是不是可信", 不做任何修改 ——
# 上一版是 `mkdir -p` → `chmod 0700` → 再校验: 根目录若是一条指向别处的符号链接, chmod
# 先跟着它把**目标目录**的权限改掉了, 校验才在后面说"不可信"。退出码对, 副作用已经造成。
_ios_offer_root_ok(){
  [[ -L "$IOS_OFFER_ROOT" ]] && return 1
  [[ -d "$IOS_OFFER_ROOT" ]] || return 1
  [[ -O "$IOS_OFFER_ROOT" ]] || return 1
  return 0
}
...
_ios_offer_session_begin(){
  ...
  # 先验, 后改。路径已存在时必须先用不跟随符号链接的判据确认可信, 确认之后才允许纠正权限;
  # 不可信的路径一个字节都不碰(不 chmod、不写入、不建子项)。
  if [[ -e "$IOS_OFFER_ROOT" || -L "$IOS_OFFER_ROOT" ]]; then
    if ! _ios_offer_root_ok; then
      _ios_offer_abort "会话目录的父目录不可信($IOS_OFFER_ROOT: 需为自己拥有的真实目录, 不能是符号链接) —— 未开放任何临时端口, 也未改动该路径。"
      return 1
    fi
    if [[ "$(stat -c %a "$IOS_OFFER_ROOT" 2>/dev/null)" != 700 ]]; then
      chmod 0700 "$IOS_OFFER_ROOT" 2>/dev/null
    fi
  else
    (umask 077 && mkdir -p "$IOS_OFFER_ROOT") 2>/dev/null
  fi
  # 复核: 创建/纠正之后再确认一次类型、归属与权限。
  if ! _ios_offer_root_ok || [[ "$(stat -c %a "$IOS_OFFER_ROOT" 2>/dev/null)" != 700 ]]; then
    _ios_offer_abort "会话目录的父目录仍不可信($IOS_OFFER_ROOT) —— 未开放任何临时端口。"
    return 1
  fi
  ...
  _IOS_OFFER_WWW="$dir"
  # 目录一落盘就立刻记一笔 staging —— 不能等 HTTP 起来才第一次记录所有权。
  # staging 记录里没有 pid, 它说的只是"这个目录归本轮所有", 不是"运行期就绪"。
  if ! _ios_offer_state_write staging "$_IOS_OFFER_SID" "" "" "$dir"; then
...
# 了, 一次都不用猜。
# 这里只认本轮那一条路径(含查询串在内逐字相等), 其余一律 404: 不列目录、不重定向、不猜
# MIME。绑定地址由调用方给(生产是 0.0.0.0, 访问范围仍由 nft 那条按源地址的放行来控)。
IOS_OFFER_SERVER='
import http.server, os, socketserver, sys, threading
PORT = int(sys.argv[1]); WANT = sys.argv[2]; FILE = sys.argv[3]; BIND = sys.argv[4]
PIDF = sys.argv[5]
with open(FILE, "rb") as fh:
    BODY = fh.read()
# 把自己的 pid 与 starttime 写进会话目录, 权限 0600。下一次会话即使拿不到运行期记录, 也能
# 从**已经证明归属的那个目录**里找到该收谁; starttime 一起记, 用来排除 PID 复用。
with open("/proc/self/stat", "rb") as fh:
    _st = fh.read().rsplit(b") ", 1)[1].split()
_fd = os.open(PIDF, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
os.write(_fd, ("pid=%d\nstart=%s\n" % (os.getpid(), _st[19].decode())).encode())
os.close(_fd)
...
t = threading.Timer(600, lambda: os._exit(0))
t.daemon = True
t.start()
S((BIND, PORT), H).serve_forever()
'

# state 必须**可信落盘**: SIGKILL 之后的自愈完全依赖它。写不进去还继续开通道, 等于把"能
# 自愈"建立在一次没发生的写入上, 而现场看不出任何异常。
# 同目录候选文件 → 写满 → chmod 0600 → 原子 rename。任何一步失败都算失败, 且不留半截文件。
# 里面是 pid 与本机路径, 所以权限固定 0600, 不跟 umask 走。
_ios_offer_state_write(){
  local phase="$1" sid="$2" pid="$3" start="$4" www="$5" dir tmp
  # phase 明确区分两种记录: staging 只说"这个目录归本轮所有"(目录一落盘就写),
  # serving 才是"运行期就绪"(带 pid 与 starttime)。不完整的记录不许冒充完整的。
  case "$phase" in staging|serving) ;; *) return 1 ;; esac
  ...
  mv -f "$tmp" "$IOS_OFFER_STATE" 2>/dev/null || { rm -f "$tmp"; return 1; }
  _IOS_OFFER_STATE_OWNED=1     # 只有写成功才算这份记录归本轮所有, teardown 才可以删它
  return 0
}
...
_ios_offer_reap_orphan(){
  # **不再以运行期记录为门。** 记录是 HTTP 起来之后才落盘的, 而目录在那之前很久就存在了;
  ...
  if [[ -d "$IOS_OFFER_ROOT" && ! -L "$IOS_OFFER_ROOT" ]]; then
    ...
    while IFS= read -r ent; do
      [[ -z "$ent" ]] && continue
      base="$(basename "$ent")"
      # 只处理**严格匹配**本功能命名的目录。名字对不上的一律不碰 —— 这个父目录里不该有
      # 别的东西, 但"不该有"不是动手的理由。
  ...
  # "不杀、不删、不覆盖" 是同一条规矩的三面, 少一面就等于替人工判断做了决定。
  if [[ "$rc" == 0 && -e "$IOS_OFFER_STATE" ]]; then
...
_ios_offer_teardown(){
  ...
  _IOS_OFFER_ACTIVE=""
  # 收尾一旦开始就必须跑完。信号处理器进来时已经把 trap 摘干净, 此后第二个 HUP/INT/TERM
...
_ios_offer_download(){
  ...
  _IOS_OFFER_SRV=$!
  if ! _ios_offer_state_write serving "$_IOS_OFFER_SID" "$_IOS_OFFER_SRV" \
...
# 已停的判据。**只有 inactive 才算停了。**
#
#   · deactivating / activating —— 还在退出中 / 正在起来。进程可能还在, 7894 可能还开着。
#     照"已停"处置就会在它还活着的时候把它的执行文件删掉: 一个没有源码可查、也没人记得
#     它存在的 MITM 还在转发流量, 那比不退役更糟;
#   · unknown / 空 / systemctl 自己失败 —— **证明不了**。证明不了就不算数;
#   · failed —— 算停了。systemd 已经把进程收掉了, 这是它自己的语义; 而且一台 pdg-mitm 早就
#     崩着的机器如果因为这个永远升不上去, 才是真的把用户卡死。
_retire_svc_stopped(){
  local st; st="$(systemctl is-active "$1" 2>/dev/null | tr -d '[:space:]')"
...
# ── 调用方能力门 ─────────────────────────────────────────────────────────────
# 它挡的是**这一版特有的不可逆动作**: 停 pdg-mitm、删 WLOC 执行件与 unit、推进 iOS 记录格式
# (推进之后带根证书的旧产物会被删掉, 回不去)。
#
# 为什么不放在 run_all_migrations 第一句: 那里还有 cmd_platform(平台切换)与 cmd_migrate
# (用户显式迁移)两个调用方, 以及新装机、已退役机器的幂等复跑 —— 它们本来就不做不可逆的事,
# 无条件拦下来只会把正常操作弄坏。所以门挂在**真正要动手之前**, 且只在"这一次确实有
# 不可逆的事要做"时才要求能力证明。
#
# 放行判据不是"环境变量设没设""文件在不在""版本号写了什么", 而是那份前像记录
# **属于我这一次操作**, 且它本身是完整、可用的:
#   · 结构完整(版本行 / 结尾行 / 条数 / 正文摘要 / 必需服务齐全且不重复);
#   · 没有任何必需服务的状态是 QUERY-FAILED —— 观测失败不能当成一种状态;
#   · boot_id 等于当前(跨重启的残留直接出局);
#   · holder_pid + holder_start 等于**我自己或我的直接调用方** —— `pdg update` 经
#     `__migrate` 子进程进来(写记录的是父进程), `pdg migrate` 在同一进程里直接调(写记录的
#     就是自己), 两种都认。上一次升级留下的记录会指向另一个进程; pid 可以复用,
#     pid+starttime 不会。这一条是"旧记录重放"的主判据;
#   · 记录所在目录里的 snap.tar.gz **不早于父进程启动** —— 连旧快照一起端过来也不行;
#   · 记录里的自启状态与**此刻仍可观测的现状**一致(这一刻还没动过任何服务)。
# 另: 用仓库既有的 _lock_inherited 佐证"本进程确实跑在继承来的锁下"。
...
_retire_caller_gate(){
  ...
  if [[ -z "$f" ]]; then
    why="调用方没有交出本次操作的服务前像句柄(PDG_UPDATE_SVCSTATE 未设)"
  elif ! _pdg_svcstate_valid "$f"; then
    why="前像记录不可用: ${_PDG_SVCSTATE_WHY:-未知}"
  elif [[ "$(stat -c %u "$f" 2>/dev/null)" != "$(id -u)" || "$(stat -c %a "$f" 2>/dev/null)" != 600 ]]; then
    why="前像记录的属主/权限不对(应属于本次执行身份 uid=$(id -u) 且 600)"
  else
    sd="$(awk -F'\t' '$1=="snap_dir"{print $2; exit}' "$f")"
    bid="$(awk -F'\t' '$1=="boot_id"{print $2; exit}' "$f")"
    pid="$(awk -F'\t' '$1=="holder_pid"{print $2; exit}' "$f")"
    pst="$(awk -F'\t' '$1=="holder_start"{print $2; exit}' "$f")"
    sid="$(awk -F'\t' '$1=="snap_id"{print $2; exit}' "$f")"
    if [[ "$sd" != "$(dirname "$f")" ]]; then
      why="前像记录与它所在目录不自洽"
    elif [[ ! -f "$sd/snap.tar.gz" ]]; then
      why="前像记录旁边没有本次快照(snap.tar.gz)"
    elif [[ "$bid" != "$(cat /proc/sys/kernel/random/boot_id 2>/dev/null)" ]]; then
      why="前像记录来自上一次开机(boot_id 不符) —— 是残留, 不是本次的"
    else
      # 写记录的必须是**我自己**或**我的直接调用方**, 两种都要认:
      #   · `pdg update` 走 `bash /usr/local/bin/pdg __migrate` 子进程 → 写记录的是父进程;
      #   · `pdg migrate` 在**同一个进程里**直接调 run_all_migrations → 写记录的就是自己。
      # 只认父进程会把 `pdg migrate` 这条正常入口误判成"别人的记录"。
      # pid 会复用, pid + starttime 不会 —— 所以两样都比。
      hstart=""
      for cand in "$$" "$PPID"; do
        [[ "$pid" == "$cand" ]] || continue
        hstart="$(awk '{print $22}' "/proc/$cand/stat" 2>/dev/null)"
        break
      done
      now="$(stat -c '%d:%i:%s:%Y' "$sd/snap.tar.gz" 2>/dev/null)"
      # 调用方进程的开机后 tick → 墙上时刻(秒, 向下取整)。/proc/<pid> 目录的 mtime **不是**
      # 进程启动时刻(每次查目录都会刷新), 不能拿它比。
      btime="$(awk '/^btime /{print $2}' /proc/stat 2>/dev/null)"
      clk="$(getconf CLK_TCK 2>/dev/null)"
      if [[ -z "$pid" || ( "$pid" != "$$" && "$pid" != "$PPID" ) ]]; then
        why="前像记录不是我、也不是我的调用方写的(记录里是 pid=${pid:-空}; 我是 $$, 我的调用方是 $PPID)"
      elif [[ -z "$pst" || -z "$hstart" || "$pst" != "$hstart" ]]; then
        why="前像记录指向的是一个已经结束的旧进程(pid 复用; 启动时刻 ${pst:-空} vs ${hstart:-空})"
      elif [[ -z "$sid" || "$sid" != "$now" ]]; then
        why="记录钉的不是现在这份快照(记录=${sid:-空}, 现在=${now:-读不到}) —— 记录与回滚要用的快照对不上"
      elif [[ -z "$btime" || -z "$clk" || "$clk" -le 0 ]]; then
        why="读不到开机时刻/时钟频率, 无法判定快照是不是本次生成的"
      elif [[ "$(stat -c %Y "$sd/snap.tar.gz" 2>/dev/null)" -lt $(( btime + hstart / clk )) ]]; then
        why="快照比写它的那个进程还早 —— 这是上一次的快照与记录被一起端过来了"
      fi
    fi
  fi
  ...
  c_y "   接下来怎么办 —— 本进程**不猜**是谁调的它: 已发布版本的 cmd_update 调新版时,"
  c_y "   它拿到的两个信号(CLI 动词 / 有没有交句柄)与用户手打 __migrate **完全一样**。"
  _retire_rerun_hint "所以下面按这台机器实际装着哪一版来说"
  ...
  c_y "   所以这不是「整机未改动」。至于这些已经装上的文件会不会被退回去 —— 本进程不知道, 也不替谁承诺:"
  c_y "   如果这一次是某个升级流程调起来的, 它接下来做什么由它自己决定, 以**它的**输出为准;"
  c_y "   如果是你直接跑的, 那就没有谁会替你退回这些文件。"
  return 1
}
...
# pdg-mitm、再被告知"归属不清, 本次未做任何改动" —— 后半句是假话, 服务已经停了。用户看到
# "未做任何改动"就不会去把它起回来, 那台机器的 WLOC 从此半死不活。
#
# 所以分成两段: ① 只读地把该拒的全拒掉、把该干的活算清楚; ② 才开始动运行态。
#
# ## 失败恢复: 按**实际改过的**逐项回退, 不是"把劫持表放回去就算零半状态"
#
# 动手那一段每改一样就往 _RETIRE_UNDO 里压一条撤销动作, 失败时倒序执行并复核。服务被停过就
# 要起回去, 内核配置换过就要换回来, mitm.json 改过就要还原 —— 只恢复一个列表而宣称"零半
# 状态"是假话。
#
# ## 保留策略(沿用项目既有那一条, 见 migrate_android_cleanup)
#
# **清运行时接管 + enabled=false, 保留地点与 CA 数据。** 用户存的地点是他自己的数据; 盘上
# 的 CA 材料只报告、不替他永久销毁 —— 销毁私钥不可逆, 而那张根证书很可能还被他手机信任着,
# 取消信任这件事只有他自己能做。
#
...
# 退役迁移失败(或被门拒)之后, 用户到底该做什么。
#
# **不猜调用者。** 本进程能看到的只有两个信号: 本次 CLI 动词, 和调用方有没有交服务前像句柄。
# 这两个信号分不出"升级调起的"和"用户手打的" —— 已发布版本的 cmd_update 调新版走的是
#     if ! bash /usr/local/bin/pdg __migrate; then
# 没有任何环境变量前缀(v1.11.15 里 PDG_UPDATE_SVCSTATE 这个名字根本不存在), 于是那个子进程
# 拿到的是 `$1=__migrate` + 句柄为空 —— 与用户手打 `sudo pdg __migrate` **完全一样**。
# 靠它们分辨只会猜错, 而猜错的代价是让一台旧机器反复去敲一条对它无效的命令。
# 也不去看进程名或 PID: 那不是可靠身份, 本版也不打算引入调用者鉴权。
#
# 所以给**条件式指引**: 把判断交给用户能实际观察、也能实际执行的那一件事 —— 这台机器最后
# 到底装着哪一版, 那一版有没有"会在动手之前保存服务前像的公开迁移入口"。
# 不替上游承诺它会不会回滚, 也不断言回退之后那一版一定没有退役能力 —— 那两件事本进程都不知道。
_retire_rerun_hint(){   # [$1=开头那句话]
  c_y "   ${1:-修好之后怎么重跑} —— 按**这台机器最后实际装着哪一版**来定, 别按这条消息猜:"
  c_y "     ① 如果你是在一次升级(sudo pdg update)里看到这条: 先让那次升级自己走完。"
  c_y "        它接下来做什么由它自己决定, 本进程既管不着也不替它承诺结果。等它结束再做 ②。"
  c_y "     ② 看**此刻**装着的那一版支不支持带服务前像的公开迁移入口:"
  c_y "          sudo grep -q _pdg_save_svcstate /usr/local/bin/pdg && echo 支持 || echo 不支持"
  c_y "     ③ 显示「支持」→ 跑 **sudo pdg migrate**: 它会先建快照、再保存服务前像, 并把句柄"
  c_y "        交给迁移, 是受支持的重跑入口。"
  c_y "        显示「不支持」→ 这台机器上**没有**命令能补上这一步。pdg __migrate 与 pdg migrate"
  c_y "        都不会有用, 别反复敲; 需要等一个在动手之前保存服务前像的升级入口装上来。"
  c_y "     · 任何情况下都不要手打 sudo pdg __migrate: 那是内部入口, 不建快照也不保存服务前像。"
  c_y "       也没有任何变量或参数可以绕过这道门。"
}
...
import bot
# 测试前缀。生产为空 ⇒ 下面这一圈是恒等变换。
R = os.environ.get("PDG_RETIRE_ROOT", "")
if R:
    for _n in ("SB", "MIHOMO_DIR", "MIHOMO_CFG", "LAN_TABLE_FILE", "RS_META",
               "MITM_CONFIG", "IOS_META", "IOS_ART_DIR"):
        _v = getattr(bot, _n, None)
        if isinstance(_v, str) and _v.startswith("/"):
            setattr(bot, _n, R + _v)
    _plat = os.path.join(R, "etc/privdns-gateway/platform")
    bot._platform = lambda p=_plat: (open(p).read().strip()
...
cmd_platform(){
  ...
  # 切 Android 经 migrate_android_cleanup)。那两步要能力门放行, 门要的是"这一次操作的前像"。
  # 前像由上面那次 cmd_snapshot 在打包之后一并存好并校验过 —— 这里**只确认, 不再采样**:
  # 再采一次会覆盖掉"动手之前"那一刻的记录, 而两次采样之间机器状态可能已经变了。
  # 确认不过就在这里停: 此刻还没有改动任何东西。
  local _psnap="${_PDG_SNAP_CREATED:-}"
  if [[ -z "$_psnap" ]] || [[ ! -f "$_psnap/svcstate.tsv" ]] || ! _pdg_svcstate_plan "$_psnap"; then
    echo "❌ 本次快照的服务前像不可用(${_PDG_SVC_WHY:-svcstate.tsv 不存在}) → 中止切换(未改动任何东西)"; return 1
  fi
  # 这次确认只为"现在能不能动手", 结论不留给后面的阶段(与 cmd_update 同一套分阶段规矩)。
  _PDG_SVC_SRC=""; _PDG_SVC_MODE=blind
  ...
  _plat_rollback(){
    ...
    while IFS='|' read -r svc en ac; do
      ...
      case "$en" in
        enabled|enabled-runtime|disabled)
          # 与 _pdg_restore_svcstate 同一份实现。**动作失败与后置不符是两个独立的失败条件**:
          # 原来这里写 `|| true`, 只看最终状态 —— 一次明确失败的动作, 只要最终值碰巧等于目标
          # 就被整笔吞掉了。最终状态一样**不是**吞掉失败的理由: 那一次动作确实没成,
          # 这台机器上发生过什么必须如实留在报告里。
          _enrc=0
          _pdg_set_enable_state "$svc" "$en" >/dev/null 2>&1 || _enrc=$?
          [[ "$_enrc" == 0 ]] || _miss+=("$svc 自启恢复动作失败(目标 $en, rc=$_enrc)");;
      esac
...
cmd_migrate(){
  ...
  # 显式迁移同样要在动手之前**确认**服务前像 —— 否则它自己就成了"不具备回滚能力的调用方",
  # 会被下面的退役门挡住。前像由上面那次 cmd_snapshot 存好并校验过, 这里**不再采样**
  # (再采一次会覆盖掉动手之前那一刻的记录)。确认不过就停在这里, 迁移一步都没开始。
  if [[ ! -f "$snap/svcstate.tsv" ]] || ! _pdg_svcstate_plan "$snap"; then
    c_y "❌ 本次快照的服务前像不可用(${_PDG_SVC_WHY:-svcstate.tsv 不存在}), 拒绝在无法完整回滚的前提下迁移。"; return 1
  fi
  _PDG_SVC_SRC=""; _PDG_SVC_MODE=blind
  PDG_UPDATE_SVCSTATE="$snap/svcstate.tsv" run_all_migrations || rc=$?
  if [[ $rc == 0 ]]; then
