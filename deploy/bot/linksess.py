...
_TWO_STEP = """请在**手机上**依次做两步(用要诊断的那张 SIM 卡, 关掉 Wi-Fi):
  ...
  第 1 步 打开这个数字地址的链接 —— 证明手机的网络能到达网关的 81:
      %s
...
说明:
  ...
  · 第 2 步的网页**最终打不开是预期结果** —— 它要的是那一次 DNS 查询本身,
    不是网页内容。
  · **本版本还收集不到第 2 步的证据**: DNS 侧的专用计数需要打开 mosdns 的
    metrics 接口, 而那个接口会连带暴露 DNS 缓存导出与投喂端点, 安全审查没让它
    过(见 tests/test-link-dns-evidence.py)。这一半留到 6.2。现在做第 2 步只是
    为了顺手排除缓存干扰, 不会产生诊断结论。
