...
说明:
  ...
  · 第 2 步的网页**最终打不开是预期结果** —— 它要的是那一次 DNS 查询本身,
    不是网页内容。
  · **当前版本无法安全取得第 2 步的证据**: DNS 侧的专用计数要打开 mosdns 的
    metrics 接口, 而同一个监听端口会连带暴露 DNS 缓存导出(内含明文查询域名)
    与缓存投喂端点, 安全审查没让它过。该能力移交 6.2 重新设计
    (见 docs/ROADMAP.md)。现在做第 2 步只是为了顺手排除缓存干扰,
    不会产生诊断结论。
