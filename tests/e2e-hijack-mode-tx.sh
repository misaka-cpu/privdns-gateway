...
out=$(PATH="$E2E_TMP/hm-bin:$PATH" pdg hijack-mode gfw 2>&1); rc=$?
...
if mount --bind /etc/privdns-gateway /etc/privdns-gateway 2>/dev/null; then
  ...
  {
    echo '#!/bin/bash'
    # 路径要在**写桩的时候**展开(桩自己跑起来时没有 $E2E_TMP), 故用双引号并转义 $*/$@
  ...
  _f=$(fp)
  out=$(PATH="$E2E_TMP/hm-stub2:$PATH" pdg hijack-mode gfw 2>&1); rc=$?
