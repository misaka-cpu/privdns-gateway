...
render(){ # $1=SSH_MATCH $2=输出路径
  sed -e "s|__SSH_PORT__|22|g" -e "s|__INTERNAL_CIDR__|172.22.0.0/16|g" \
      -e "s|__SSH_MATCH__|$1|g" -e "s|__RESCUE_PORT__|$RPORT|g" "$TPL" > "$2"
}
...
# 这一格是本文件存在的全部理由。
render 'iifname "tailscale0" ' "$WORK/b.conf"
...
PY
render 'iifname "tailscale0" ' "$WORK/e.conf"
