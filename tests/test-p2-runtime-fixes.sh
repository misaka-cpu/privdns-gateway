...
OLD="$WORK/pdg-old.sh"
git -C "$ROOT" show HEAD:deploy/bot/pdg.sh > "$OLD" 2>/dev/null || : > "$OLD"
...
mkrepo(){                               # $1=HEAD 是否落在最新 tag 上; 打印仓库路径
  ...
  git -C "$d" -c user.email=t@t -c user.name=t commit -q --allow-empty -m c1
  # 用**附注** tag: 轻量 tag 的对象哈希就是提交, 少写 `^{commit}` 也能碰巧相等 ——
  # 那样这一格就测不出真正的错误形态了。
  git -C "$d" -c user.email=t@t -c user.name=t tag -a v9.9.9 -m r 2>/dev/null
  [[ "$1" == "behind" ]] && git -C "$d" -c user.email=t@t -c user.name=t commit -q --allow-empty -m c2
  echo "$d"
}
...
git -C "$dirty" -c user.email=t@t -c user.name=t add dirty.txt 2>/dev/null
git -C "$dirty" -c user.email=t@t -c user.name=t commit -q -m d 2>/dev/null
git -C "$dirty" -c user.email=t@t -c user.name=t tag -f -a v9.9.9 -m r 2>/dev/null
