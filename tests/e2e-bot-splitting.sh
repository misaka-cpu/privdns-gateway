...
botpy > $E2E_TMP/e2e-bs1.out 2>&1 <<'PY'
import base64, sys
...
PY
grep -q "^ADD_EXIT True" $E2E_TMP/e2e-bs1.out && ok "加落地出口 e-ss" || bad "加出口失败: $(head -2 $E2E_TMP/e2e-bs1.out)"
grep -q "^ADD_RULE True" $E2E_TMP/e2e-bs1.out && ok "把 perfops2.byte-test.example 指到 e-ss" \
  || bad "加规则失败: $(grep ADD_RULE $E2E_TMP/e2e-bs1.out)"
...
bash /usr/local/bin/pdg hijack-mode gfw >$E2E_TMP/e2e-hm.log 2>&1
rc=$?
[[ "$rc" == 0 ]] && ok "切到 gfw 模式" || bad "切 gfw 失败(rc=$rc): $(tail -2 $E2E_TMP/e2e-hm.log)"
EP="$(epline)"; CN="$(cnline)"
{ [[ -n "$EP" && -n "$CN" ]] && [[ "$EP" -lt "$CN" ]]; } \
...
bash /usr/local/bin/pdg hijack-mode all >$E2E_TMP/e2e-hm2.log 2>&1
rc=$?
[[ "$rc" == 0 ]] && ok "切回 all 模式" || bad "切回 all 失败(rc=$rc): $(tail -2 $E2E_TMP/e2e-hm2.log)"
EP="$(epline)"; CN="$(cnline)"
{ [[ -n "$EP" && -n "$CN" ]] && [[ "$EP" -lt "$CN" ]]; } \
...
botpy > $E2E_TMP/e2e-bs4.out 2>&1 <<'PY'
import sys
sys.path.insert(0, "/opt/pdg-bot")
import bot
...
PY
grep -q "^TO_DIRECT True" $E2E_TMP/e2e-bs4.out && ok "改判直连成功" || bad "$(grep TO_DIRECT $E2E_TMP/e2e-bs4.out)"
grep -q "^IN_HIJACK False" $E2E_TMP/e2e-bs4.out && ok "旧的劫持记录被清掉" || bad "劫持记录还在"
grep -q "^IN_DIRECT True" $E2E_TMP/e2e-bs4.out && ok "进了直连表" || bad "没进直连表"
reload
...
botpy > $E2E_TMP/e2e-bs5.out 2>&1 <<'PY'
import sys
sys.path.insert(0, "/opt/pdg-bot")
import bot
...
PY
g(){ grep -q "^$1" $E2E_TMP/e2e-bs5.out; }
g "WDA_ON True"           && ok "开启 WDA" || bad "开 WDA 失败: $(grep WDA_ON $E2E_TMP/e2e-bs5.out)"
g "WDA_STATE_1 True"      && ok "开启后状态为真" || bad "开启后状态不对"
g "ADD_JP True"           && ok "加普通 jp 分流" || bad "$(grep ADD_JP $E2E_TMP/e2e-bs5.out)"
g "MINE_IN_WDA False"     && ok "用户域名没有被并进 WDA 规则" || bad "用户域名被并进 WDA 规则了"
grep -qE "^WDA_RULE_LEN 1 [0-9]+" $E2E_TMP/e2e-bs5.out \
  && ok "WDA 规则仍是单独一条(域名数 $(awk '/^WDA_RULE_LEN/{print $3}' $E2E_TMP/e2e-bs5.out))" \
  || bad "WDA 规则数量不对: $(grep WDA_RULE_LEN $E2E_TMP/e2e-bs5.out)"
...
botpy > $E2E_TMP/e2e-bs6.out 2>&1 <<'PY'
import sys
sys.path.insert(0, "/opt/pdg-bot")
import bot
...
PY
grep -q "^UPSTREAM True" $E2E_TMP/e2e-bs6.out && ok "改 local 上游成功" \
  || bad "改上游失败: $(grep UPSTREAM $E2E_TMP/e2e-bs6.out)"
EP="$(epline)"; CN="$(cnline)"
{ [[ -n "$EP" && -n "$CN" ]] && [[ "$EP" -lt "$CN" ]]; } \
...
botpy > $E2E_TMP/e2e-bs8a.out 2>&1 <<'PY'
import sys
sys.path.insert(0, "/opt/pdg-bot")
import bot
...
PY
if grep -q "^RENAME None" $E2E_TMP/e2e-bs8a.out; then
  echo "  [--] bot 没有 rename_exit, 跳过改名(由 test-exit-rename.py 覆盖)"
else
  grep -q "^RENAME True" $E2E_TMP/e2e-bs8a.out && ok "8a 出口改名成功" || bad "8a 改名失败: $(grep RENAME $E2E_TMP/e2e-bs8a.out)"
  grep -q "^DANGLING \[\]" $E2E_TMP/e2e-bs8a.out \
    && ok "8a 改名后没有指向不存在出口的悬空规则" || bad "8a 悬空规则: $(grep DANGLING $E2E_TMP/e2e-bs8a.out)"
  grep -q "^HIJACK_KEPT True" $E2E_TMP/e2e-bs8a.out \
...
echo
botpy > $E2E_TMP/e2e-bs8b.out 2>&1 <<'PY'
import sys
sys.path.insert(0, "/opt/pdg-bot")
import bot
bot._wda_authorized = lambda: True
...
print("NF_IN_WDA_RULE", any("netflix.com" in (r.get("domain_suffix") or [])
                            for r in bot.load()["route"]["rules"] if r.get("outbound") == "jp"))
PY
grep -q "^NF_DIRECT True" $E2E_TMP/e2e-bs8b.out && ok "8b 把 WDA 域名改判直连: 操作被接受" \
  || ok "8b 把 WDA 域名改判直连: 被拒绝($(grep NF_DIRECT $E2E_TMP/e2e-bs8b.out | cut -d' ' -f3-))"
reload
NFANS="$(q netflix.com)"
if grep -q "^NF_IN_DIRECT True" $E2E_TMP/e2e-bs8b.out; then
  ...
  grep -q "^WDA_STILL_ON True" $E2E_TMP/e2e-bs8b.out \
...
bot.set_wda_mode(True)
PY
bash /usr/local/bin/pdg hijack-mode gfw >$E2E_TMP/e2e-hm3.log 2>&1
botpy > $E2E_TMP/e2e-bs8c.out 2>&1 <<'PY'
import sys
sys.path.insert(0, "/opt/pdg-bot")
import bot
...
PY
grep -q "^WDA_AFTER_MODE True" $E2E_TMP/e2e-bs8c.out \
  && ok "8c 切劫持模式后 WDA 状态未被冲掉" || bad "8c 切模式把 WDA 弄丢了"
grep -q "^WDA_RULES 1" $E2E_TMP/e2e-bs8c.out \
  && ok "8c 切模式后 WDA 规则仍是一条(没被复制)" || bad "8c WDA 规则数: $(grep WDA_RULES $E2E_TMP/e2e-bs8c.out)"
EP="$(epline)"; CN="$(cnline)"
{ [[ -n "$EP" && -n "$CN" ]] && [[ "$EP" -lt "$CN" ]]; } \
...
echo
botpy > $E2E_TMP/e2e-bs8d.out 2>&1 <<'PY'
import sys
sys.path.insert(0, "/opt/pdg-bot")
import bot
...
PY
if grep -q "^AFTER_TAGS" $E2E_TMP/e2e-bs8d.out; then
  grep -q "^DANGLING \[\]" $E2E_TMP/e2e-bs8d.out \
    && ok "8d 删出口后没有指向不存在出口的悬空规则" \
    || bad "8d 悬空规则: $(grep DANGLING $E2E_TMP/e2e-bs8d.out)"
  echo "  [信息] 原本指向被删出口的规则改指到: $(awk '/^ORPHAN_TARGET/{print $2}' $E2E_TMP/e2e-bs8d.out)" \
       "(final=$(awk '/^FINAL/{print $2}' $E2E_TMP/e2e-bs8d.out))"
  # 规则被改指到 final 而不是删除, 所以劫持表里留着该域名是**一致**的 —— 但如果 final 是
  # direct, 这个域名就变成"DNS 劫到网关 + 内核判直连", 与用户在直连表里看到的不是一回事。
  if grep -q "^HIJACK_LEFT True" $E2E_TMP/e2e-bs8d.out; then
    if grep -q "^ORPHAN_TARGET \['direct'\]" $E2E_TMP/e2e-bs8d.out; then
  ...
  bad "8d 删出口路径没跑起来: $(head -3 $E2E_TMP/e2e-bs8d.out)"
fi

# 8e. 规则集: 从本地 HTTP 源真加一个 .list, 看它进不进 mihomo 运行配置
echo
python3 - > $E2E_TMP/e2e-rs-srv.log 2>&1 <<'PY' &
...
[[ "$RSUP" == 1 ]] && ok "8e 本地规则集源已就绪" \
  || bad "8e 本地规则集源没起来: $(tail -3 $E2E_TMP/e2e-rs-srv.log 2>/dev/null | tr '\n' ' ')"
botpy > $E2E_TMP/e2e-bs8e.out 2>&1 <<'PY'
import sys
sys.path.insert(0, "/opt/pdg-bot")
import bot
...
PY
if grep -q "^ADD_RS True" $E2E_TMP/e2e-bs8e.out; then
  ok "8e 规则集加入成功"
  NAME="$(python3 -c "
import re,sys
m=re.search(r\"^RS_META \\[(.*)\\]\", open(\"$E2E_TMP/e2e-bs8e.out\").read(), re.M)
print((m.group(1).split(',')[0].strip().strip(chr(39))) if m else '')")"
  ...
  botpy > $E2E_TMP/e2e-bs8f.out 2>&1 <<'PY'
import sys
sys.path.insert(0, "/opt/pdg-bot")
import bot
...
PY
  grep -q "^DEL_RS True" $E2E_TMP/e2e-bs8f.out && ok "8e 删除规则集成功" || bad "8e $(grep DEL_RS $E2E_TMP/e2e-bs8f.out)"
  ...
  printf 'mrsonly.example\n+.mrssuf.example\n' > $E2E_TMP/e2e-mrs-src.txt
  if mihomo convert-ruleset domain text $E2E_TMP/e2e-mrs-src.txt /etc/sing-box/rs/rs_mrs.mrs >/dev/null 2>&1; then
    ok "8g 造出真 .mrs 规则集"
    botpy > $E2E_TMP/e2e-bs8g.out 2>&1 <<'PY'
import json, os, sys
sys.path.insert(0, "/opt/pdg-bot")
import bot
m = dict(bot._rs_meta())
...
PY
    grep -q "^MRS_TX True" $E2E_TMP/e2e-bs8g.out && ok "8g .mrs 规则集提交成功" \
      || bad "8g $(grep MRS_TX $E2E_TMP/e2e-bs8g.out)"
    grep -q "^MRS_UNDRIVABLE \[\]" $E2E_TMP/e2e-bs8g.out \
      && ok "8g .mrs 不再被算作「派生不了」" || bad "8g $(grep MRS_UNDRIVABLE $E2E_TMP/e2e-bs8g.out)"
    reload
  ...
  rm -f $E2E_TMP/e2e-mrs-src.txt
  bash /usr/local/bin/pdg hijack-mode all >/dev/null 2>&1; reload
else
  bad "8e 规则集加入失败: $(grep ADD_RS $E2E_TMP/e2e-bs8e.out)"
  echo "  源服务日志: $(tail -3 $E2E_TMP/e2e-rs-srv.log 2>/dev/null | tr '\n' ' ')"
fi
...
cp /etc/mosdns/config.yaml $E2E_TMP/e2e-oldmos.yaml
python3 "$E2E_ROOT/tests/helpers/strip-explicit-proxy.py" $E2E_TMP/e2e-oldmos.yaml \
  && ok "9 造出一份不含明确代理的旧备份" || bad "9 夹具没造对"
botpy > $E2E_TMP/e2e-bs9.out 2>&1 <<'PY'
import io, os, sys, tarfile
sys.path.insert(0, "/opt/pdg-bot")
import bot
TMP = os.environ["E2E_TMP"]          # 引号 heredoc 不展开变量, 从环境里取
buf = io.BytesIO()
...
PY
grep -q "^RESTORE True" $E2E_TMP/e2e-bs9.out && ok "9 恢复成功" || bad "9 恢复失败: $(grep MSG $E2E_TMP/e2e-bs9.out)"
...
grep -q "旧版本" $E2E_TMP/e2e-bs9.out \
...
python3 /opt/pdg-bot/doctor.py --json > $E2E_TMP/e2e-doc9.json 2>/dev/null
python3 "$E2E_ROOT/tests/helpers/doctor-explicit-proxy.py" $E2E_TMP/e2e-doc9.json ok \
  && ok "9 恢复后 doctor 仍判 ok" || bad "9 恢复后 doctor: $(head -c 160 $E2E_TMP/e2e-doc9.json)"

# ══ 7. doctor 对最终状态的判断 ═════════════════════════════════════════════════
echo; echo "── 7. doctor ──"
python3 /opt/pdg-bot/doctor.py --json > $E2E_TMP/e2e-doc.json 2>/dev/null
python3 "$E2E_ROOT/tests/helpers/doctor-explicit-proxy.py" $E2E_TMP/e2e-doc.json ok \
  && ok "doctor: 指定域名优先级 = ok" || bad "doctor 判定不对: $(head -c 200 $E2E_TMP/e2e-doc.json)"
