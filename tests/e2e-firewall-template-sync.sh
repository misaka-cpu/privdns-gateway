...
sed -e "s|__SSH_PORT__|$port|g" -e "s|__SSH_MATCH__||g" -e "s|__TAILNET_DIRECT__|# (SSH 未收紧为 tailnet, 故不放行 Tailscale 直连端口)|g" -e "s|__INTERNAL_CIDR__|$cidr|g" -e "s|__RESCUE_PORT__|$rport|g" \
