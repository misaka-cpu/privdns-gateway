...
#
# `lis` 必须**精确**取"监听门"那一条, 不能拿 "5399" in l 一网打尽: 归属校验(端口
# 被别的进程占着)也带 5399, 但它是第三种情况 —— 进程起来了、端口也有人听, 只是听的
# 人不是我们。把它卷进这一组, 分组的第一条就可能变成归属那句, 于是这道门比的是两件
# 无关的事; 更糟的是负控改坏后会先撞上上面"共用同一句理由"那条, 红是红了却点错了名。
LISTEN_GATE = "已启动但没有在"          # 监听门那句的稳定片段
val = [l for _, l in exits if "校验" in l]
lis = [l for _, l in exits if LISTEN_GATE in l]
