...
def main():
    ...
    bot._platform = lambda: "android"
    # wloc:* 仍在名单里: 那些按钮还留在用户的旧 TG 消息里, 点下去会走**退役提示**那一支。
    # 判据不变 —— 无论走门控还是走提示, 都不该有文件被发出去。
    ...
    ok("Android: ios/iosgen/iosgenca 被门控、wloc:* 回退役提示(send_document 从未调用)")
    ...
    # ── WLOC 后端已整体退役 ──
    # 这里原本逐个验 Android 上 wloc_add/del/switch/enable/set_wloc/_mitm_transact 全部拒绝。
    # 那些函数不再存在, 于是"平台门控有没有生效"这个问题对它们已经没有意义。改成更强的判据:
    # 它们**一个都不该回来** —— 平台门控只能挡住调用, 而函数不存在是两个平台一起管住。
    for gone in ("wloc_add", "wloc_del", "wloc_switch", "wloc_enable", "set_wloc",
                 "_mitm_transact", "_mitm_enabled_domains", "_mitm_domains"):
        assert not hasattr(bot, gone), f"{gone} 又回来了 —— WLOC 后端应已整体退役"
    ok("Android+iOS: WLOC 后端函数一个都不在(退役比平台门控更彻底)")
    ...
    # 这里原本验的是"WLOC 开着却读不到 CA → 拒绝生成描述文件"。退役后描述文件**恒不含**
    # 根证书, 那个分支不存在了; 换成盯住退役这一侧: 生成得出来, 且里面没有根证书 payload。
    prof = bot._ios_profile()
    assert prof and b"PayloadContent" in prof, "iOS _ios_profile() 应正常生成"
    assert b"com.apple.security.root" not in prof, "退役后描述文件不该再带根证书 payload"
    assert bot._ios_ca() == (False, b""), "_ios_ca 应恒为 (False, b'')"
    _, opskb = bot._nav("ops")
    assert not any("位置改写" in t for t in kb_texts(opskb)), "运维菜单不该再有 WLOC 入口"
    ok("iOS: _ios_profile 正常生成且不含根证书; 运维菜单已无 WLOC 入口")
