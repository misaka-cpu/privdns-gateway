...
# 只数**入站包的源地址**, 不再把整行里的地址一视同仁地抓出来。
#
# 起因: Tailscale 的 DERP 中继走 TCP 443, 正好落在下面的端口集里。那些包的对端是公网,
# 会被后面的私网/CGNAT 分类过滤掉 —— 但**本机自己的地址每个包都要记一次**(它在入站包里
# 是目的、在出站包里是源, 两头都被 `grep -oE` 捞走)。实测 40 秒里本机地址拿到 324 票、
# tailnet 只有 176 票, 于是探测结果被拽向"本机所在段"。接口排除挡得住选中 tailnet, 挡不住
# 这个偏置 —— 它来自 eth0 上的流量。
#
# 语义上本来要找的就是"手机**打进来**的包的源地址", 一直没写出来而已。`-i any` 的新格式是
#   <时刻> <接口> <方向> IP <源>.<端口> > <目的>.<端口>: …
# 所以 $3=="In" 且取 $5 就精确对上: 本机地址不再进候选(入站包里它是 $7), 出站包整行不计。
#
# 只在拿得到接口名时这么做。老 tcpdump 的 -i any 不打接口名, 方向字段也就无从定位 ——
# 那时沿用原来的整行提取, 与本改动前逐字等价(没装 Tailscale 的机器本来也没有 DERP 偏置)。
if [[ "$ifname_ok" == 1 ]]; then
  EXTRACT='$2 != ts && $3 == "In" { print $5 }'
else
  EXTRACT='$2 != ts { print }'
fi
