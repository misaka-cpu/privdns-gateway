...
手机那一段需要 `sudo pdg link session start`：它给出一条一次性链接，你用要诊断的那张 SIM
（关掉 Wi-Fi）在手机上打开，服务器就能记下这次请求。
...
没有证据也不等于手机故障，只说明这次没有观察到。诊断会话只保存请求来源的 `/16` 网段，
不保存完整 IP。
