...
内核那一侧同理：规则表**自上而下、第一条命中即止**，所以顺序就是优先级。项目会自动往规则表里
塞两批规则，处置不一样：

- **🔓 WDA 解锁**（那批流媒体/AI 域名 → `jp` 直出，内核里显示 `DIRECT`）排在**你点名的单域名
  规则之后**——你把 `netflix.com` 指到某个出口，它就走那个出口，WDA 只管你没点名的那些。
- **MITM 接管**（iOS 的 `gs-loc*.apple.com` → `MITM-OUT`）**故意**排在最前：WLOC 要先终止 TLS
  才改得了坐标，让路就等于关掉这个功能。

两种情况都不会"悄悄"发生：`sudo pdg doctor` 的「分流优先级」会列出 WDA 自动判成直连的那一批、
其中哪几个按你自己的规则走、哪几个因为在直连表里而对 WDA 不生效，以及**任何一条永远轮不到的
点名规则**（含被 MITM 接管盖住的）。自动加的规则用户没写过，至少得看得见。
