...
- Android：手机在系统「私密 DNS」里直接填 DoT 域名。不安装 iOS 描述文件与 pdg-probe81。
- iOS：通过 iOS 描述文件接入，另外安装 pdg-probe81（`:81` 探测）。
  （MITM/WLOC 组件已退役，两个平台都不再安装 —— 见第 10 节。）
...
## 10. iOS 位置改写（WLOC）— 已退役

WLOC 位置改写，连同它专属的 MITM 执行能力（`pdg-mitm` 服务、自签根 CA、`gs-loc` 流量接管），已从项目中移除。新安装不包含它；升级会在老机器上停掉服务、撤掉专属的 DNS 劫持与内核路由。

**如果你之前用过它，请做两件事：**

1. 在 Bot「📱 客户端 → iOS 描述文件」里**重新生成并安装**描述文件 —— 新的那份不再包含根证书。
2. 在 iPhone「设置 → 通用 → 关于本机 → 证书信任设置」里**取消对 PrivDNS Gateway MITM CA 的信任**，再到「设置 → 通用 → VPN 与设备管理」删掉旧的描述文件。

第 2 步必须你自己在手机上做：网关退役不会、也无法取消手机上已经给出的信任。在你取消之前，那张根证书对这台手机仍然有效。

网关上 `/etc/privdns-gateway/ca/` 里的 CA 材料**不会被自动删除**（销毁私钥不可逆，而且它可能仍被别处引用）。升级时会提示它还在；确认之后可以自行删除该目录。

你存过的地点坐标保留在 `/etc/privdns-gateway/mitm.json` 里，不再被任何东西读取。

DNS、分流、代理、iOS 描述文件的其余功能都不受影响。设计与实现记录见 [docs/design-mitm-plugins.md](docs/design-mitm-plugins.md)（**历史文档**）。
...
| | 层 | 谁能压过它 |
|---|---|---|
| 1 | **基础设施**(本机 DoT 域名、面板域名、更新源、证书与 ACME) | 首版谁都不能 |
...
主菜单有 `🛡 去广告`,点进去是五项:**当前状态 / 添加阻断规则 / 删除阻断规则 / 查询域名 / 返回**。
加规则的流程与 bot 里其它按钮一样:点「添加阻断规则」,再把域名作为下一条消息发出去。
**没有新增任何 `/adblock_*` 命令** —— 全部走内联按钮。

几个值得先知道的行为:

- **去广告没启用时也能加规则。**规则会存进你的 block 文件,bot 会明说「已保存,但去广告
  当前未启用,因此尚未生效」——不会替你把功能打开。
- **删除只删这个入口加的那一条**,也就是规范化之后完全相等的 `domain:<域名>` 行。你自己
  手写的 `full:` 规则、裸域名、注释、父域与子域,一律不动。规则本来就不在,会如实回你
  「原本不存在」,不写文件也不重启。
- **重复添加是空操作**,不会重启 mosdns。只有编译产物真的变了才会重启一次。
- **启用状态下失败会整份回滚**,你的规则文件与已下载的表都退回操作前的样子;万一回滚
  本身没做完,bot 会单独告诉你需要上网关人工核对,而不是含糊报个失败。
- **群里谁发起的,只有谁能完成。**别人在同一个群里发的下一条消息不算数。
- 按钮里**没有**启用、停用、更新第三方表 —— 那三件事会改变整台网关的解析行为并重启
  mosdns,留在终端上做:`sudo pdg adblock enable|disable|update`。

底层是两条可信命令,bot 只经它们改规则,自己不碰规则文件:

```bash
sudo pdg adblock rule-add <域名>
sudo pdg adblock rule-del <域名>
```

它们会取全局配置锁,输出一行 JSON 结果码(`applied` / `saved_inactive` / `already_exists` /
`not_found` / `invalid_input` / `apply_failed_rolled_back` / `rollback_incomplete`),
在终端上直接跑也一样能用。
...
| 层 | 组件 | 说明 |
|---|---|---|
| DNS | mosdns v5 | 按来源 IP 分支；判断顺序为强制接管集(`force_hijack`，WLOC 退役后常空) → **你指定要走出口的域名** → 国内直连 → 自动海外判断；代理域名 A 记录改写到本机、AAAA / HTTPS 置空；ECS 处理；缓存；DoT（853）；可选 GFWList 劫持模式 |
| 流量 | mihomo（clash.meta） | nft REDIRECT 入站 + redir 监听 + SNI 嗅探。多出口故障切换；提供 clash_api（观测面板）。改配置前先校验，失败回滚 |
| 管理 | Telegram Bot（Python 标准库） | 出口、分流、规则集、测速、流量、备份恢复、iOS 描述文件、自定义域名；改配置前先校验，失败回滚 |
...
- [docs/design-mitm-plugins.md](docs/design-mitm-plugins.md) — iOS 位置改写（WLOC）设计与原理（**该功能已退役**，保留作历史记录）
